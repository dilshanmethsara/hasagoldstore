...
import { Bell, Gift, Menu, Search, X, Smartphone } from "lucide-react";
...
function DashboardLayout() {
  ...
  // Redirect unverified users to verify-email page while keeping the dashboard shell intact.
  useEffect(() => {
    ...
    <div className="flex min-h-screen w-full bg-background relative overflow-hidden">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border/40 bg-background/50 px-4 backdrop-blur-xl sm:px-6">
          ...
          <div className="ml-auto flex items-center gap-2">
            <button
              ...
              className="grid h-10 w-10 place-items-center rounded-xl border border-white/10 bg-white/5 text-muted-foreground hover:text-foreground hover:bg-white/[0.08] transition-all"
            ...
            <div className="flex items-center gap-2.5 rounded-xl border border-white/10 bg-white/5 px-2.5 py-1.5">
              ...
              <div className="hidden text-left sm:block">
                ...
                <p className="text-[10px] leading-tight text-primary font-bold">{phoneVerified ? "Verified User" : "Phone pending"}</p>
              </div>
            </div>
          </div>
        </header>
        ...
        <main className="flex-1 px-4 py-6 sm:px-6 lg:px-8 lg:py-8 overflow-y-auto">
          {!phoneVerified && user ? (
            <div className="mb-4 flex items-center justify-between rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-300">
              <span>Secure your account by verifying your mobile number for protected actions.</span>
              <button type="button" onClick={() => setVerifyOpen(true)} className="font-semibold text-amber-200 hover:underline">Verify now</button>
            </div>
          ) : null}
          <Outlet />
        </main>
