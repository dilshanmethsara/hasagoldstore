...
import { Mail, ArrowRight, RotateCcw, AlertCircle } from "lucide-react";
...
function VerifyEmailPage() {
  ...
  const handleVerify = async () => {
    ...
    setVerifying(true);
    try {
      await http.post<{ ok: true }>("/auth/verify-email", { token: otp });
      toast.success("Email verified! Welcome to HASA GOLD STORE");
  ...
  const handleResend = async () => {
    ...
    setResending(true);
    try {
      await http.post<{ ok: true }>("/auth/verify-email/resend", { email });
  ...
  return (
    ...
    >
      <div className="space-y-6">
        ...
        >
          {verifying ? "Verifying..." : <>Verify email <ArrowRight className="h-4 w-4" /></>}
        </Button>
